...
const getTranslation = (key, locale) => {
  return props.serviceType[locale]?.[key] || '-';
};
</script>
          ...
          <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            ...
            <span
              :class="[
                ...
                serviceType.status ? 'bg-[#d4edda] text-[#155724]' : 'bg-[#f8d7da] text-[#721c24]'
              ]"
            >
              ...
              <div class="flex-1 space-y-4">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    ...
                    <code class="bg-[#f8f9fa] px-2 py-1 rounded text-[#1f2d3d]">{{ serviceType.slug }}</code>
                  </div>
                  <div class="flex items-center justify-between py-2 border-b border-gray-100">
        ...
        <div class="bg-white rounded shadow-sm">
          ...
          <div class="p-4">
            ...
            <div v-show="activeTab === 'uz'" class="space-y-4">
              <div>
                <label class="block text-xs font-medium text-[#6c757d] uppercase tracking-wide mb-1">Nomi</label>
                <p class="text-lg font-semibold text-[#1f2d3d]">{{ getTranslation('name', 'uz') }}</p>
              </div>
              <div v-if="getTranslation('description', 'uz') !== '-'">
                <label class="block text-xs font-medium text-[#6c757d] uppercase tracking-wide mb-1">Tavsifi</label>
                <p class="text-[#1f2d3d] leading-relaxed">{{ getTranslation('description', 'uz') }}</p>
              </div>
              <div v-else class="text-sm text-[#6c757d] italic">Tavsif kiritilmagan</div>
            </div>

            <!-- Russian -->
            <div v-show="activeTab === 'ru'" class="space-y-4">
              <div>
                <label class="block text-xs font-medium text-[#6c757d] uppercase tracking-wide mb-1">Название</label>
                <p class="text-lg font-semibold text-[#1f2d3d]">{{ getTranslation('name', 'ru') }}</p>
              </div>
              <div v-if="getTranslation('description', 'ru') !== '-'">
                <label class="block text-xs font-medium text-[#6c757d] uppercase tracking-wide mb-1">Описание</label>
                <p class="text-[#1f2d3d] leading-relaxed">{{ getTranslation('description', 'ru') }}</p>
              </div>
              <div v-else class="text-sm text-[#6c757d] italic">Описание не заполнено</div>
            </div>

            <!-- English -->
            <div v-show="activeTab === 'en'" class="space-y-4">
              <div>
                <label class="block text-xs font-medium text-[#6c757d] uppercase tracking-wide mb-1">Name</label>
                <p class="text-lg font-semibold text-[#1f2d3d]">{{ getTranslation('name', 'en') }}</p>
              </div>
              <div v-if="getTranslation('description', 'en') !== '-'">
                <label class="block text-xs font-medium text-[#6c757d] uppercase tracking-wide mb-1">Description</label>
                <p class="text-[#1f2d3d] leading-relaxed">{{ getTranslation('description', 'en') }}</p>
              </div>
              <div v-else class="text-sm text-[#6c757d] italic">Description not provided</div>
            </div>
          </div>
        </div>
      </div>
