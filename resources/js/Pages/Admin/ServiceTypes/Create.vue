...
const form = useForm({
  ...
  uz: { name: '', description: '' },
  ru: { name: '', description: '' },
  durations: [{ duration: 60, price: 100000, is_default: true, status: true }],
});

const addDuration = () => {
  form.durations.push({ duration: 60, price: 100000, is_default: false, status: true });
};
...
const setDefault = (index: number) => {
  form.durations.forEach((d, i) => { d.is_default = i === index; });
};
...
const submit = () => {
  form.post(route('admin.service-types.store'), { preserveScroll: true });
};
</script>
...
<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-bold tracking-tight">{{ t('serviceTypes.new', 'Yangi xizmat turi') }}</h1>
      <p class="text-muted-foreground">{{ t('serviceTypes.newSubtitle', 'Yangi massaj turini yaratish') }}</p>
    </div>
        ...
        <div class="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p class="mt-2 text-xs text-muted-foreground">Max: 2MB. Format: JPG, PNG, GIF</p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent class="pt-6 space-y-2">
              ...
              <Button type="button" variant="outline" class="w-full" as-child>
                <Link href="/admin/service-types">{{ t('common.cancel', 'Bekor qilish') }}</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
