  ...
  <div>
    ...
    <div class="mb-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-[#1f2d3d]">{{ t('testimonials.new') }}</h1>
          <p class="text-sm text-[#6c757d] mt-1">{{ t('common.create') }} {{ t('testimonials.singular').toLowerCase() }}</p>
        </div>
              ...
              <div v-for="tab in tabs" :key="tab.key" v-show="activeTab === tab.key" class="space-y-4">
                <div>
                  <label class="block text-sm font-medium text-[#1f2d3d] mb-1">
                    {{ t('testimonials.clientName') }} <span class="text-[#dc3545]">*</span>
                  </label>
                  <input
                    type="text"
                    v-model="form[tab.key].client_name"
                  ...
                  />
                  <div v-if="form.errors[`${tab.key}.client_name`]" class="text-[#dc3545] text-xs mt-1">{{ form.errors[`${tab.key}.client_name`] }}</div>
                </div>
                <div>
                  <label class="block text-sm font-medium text-[#1f2d3d] mb-1">{{ t('testimonials.clientRole') }}</label>
                  <input
                    type="text"
                    v-model="form[tab.key].client_role"
                    class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-[#007bff] focus:border-[#007bff] text-sm"
                    :placeholder="t('testimonials.enterRole')"
                  />
                </div>
                <div>
                  <label class="block text-sm font-medium text-[#1f2d3d] mb-1">
                    {{ t('testimonials.comment') }} <span class="text-[#dc3545]">*</span>
                  </label>
                  <textarea
                    v-model="form[tab.key].comment"
                    rows="5"
                    class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-[#007bff] focus:border-[#007bff] text-sm"
                    :placeholder="t('testimonials.enterComment')"
                  ></textarea>
                  <div v-if="form.errors[`${tab.key}.comment`]" class="text-[#dc3545] text-xs mt-1">{{ form.errors[`${tab.key}.comment`] }}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-white rounded shadow-sm">
            <div class="px-4 py-3 border-b border-gray-200">
              <h3 class="font-semibold text-[#1f2d3d] text-sm">{{ t('testimonials.rating') }}</h3>
            </div>
            <div class="p-4">
              <div class="flex items-center gap-1">
                <button
                  ...
                  :class="s <= form.rating ? 'text-[#ffc107]' : 'text-gray-300'"
                >
                  ★
                </button>
              </div>
              <p class="text-xs text-[#6c757d] mt-2">{{ t('testimonials.ratingHint') }}</p>
              <div v-if="form.errors.rating" class="text-[#dc3545] text-xs mt-1">{{ form.errors.rating }}</div>
            </div>
          </div>
          ...
          <div class="bg-white rounded shadow-sm">
            <div class="px-4 py-3 border-b border-gray-200">
              <h3 class="font-semibold text-[#1f2d3d] text-sm">{{ t('testimonials.sortOrder') }}</h3>
            </div>
            <div class="p-4">
              <input
                type="number"
                ...
                class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-[#007bff] focus:border-[#007bff] text-sm"
              />
              <p class="text-xs text-[#6c757d] mt-2">{{ t('testimonials.sortOrderHint') }}</p>
            </div>
          </div>
          ...
          <div class="bg-white rounded shadow-sm">
            ...
            <div class="p-4">
              ...
              <p class="text-xs text-[#6c757d] mt-2">{{ t('testimonials.activeHint') }}</p>
            </div>
          </div>
