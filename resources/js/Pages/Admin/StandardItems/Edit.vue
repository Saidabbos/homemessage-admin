  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground">{{ item.uz?.name || item.slug }}</p>
      </div>
      <Button variant="outline" as-child>
        <Link :href="route('admin.standard-items.index')">{{ t('common.back', 'Orqaga') }}</Link>
      </Button>
    </div>
    ...
    <form @submit.prevent="submit">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent>
              ...
              <div v-show="activeTab === 'uz'" class="space-y-4">
                <div class="space-y-2">
                  ...
                  <Input v-model="form.uz.name" placeholder="Masalan: Sochiq" />
                  <p v-if="form.errors['uz.name']" class="text-destructive text-xs">{{ form.errors['uz.name'] }}</p>
                </div>
                <div class="space-y-2">
                  <Label>Tavsifi (O'zbek)</Label>
                  <Textarea v-model="form.uz.description" rows="3" placeholder="Element haqida..." />
                </div>
              </div>
              ...
              <div v-show="activeTab === 'ru'" class="space-y-4">
                <div class="space-y-2">
                  ...
                  <Input v-model="form.ru.name" placeholder="Например: Полотенце" />
                  <p v-if="form.errors['ru.name']" class="text-destructive text-xs">{{ form.errors['ru.name'] }}</p>
                </div>
                <div class="space-y-2">
                  <Label>Описание (Русский)</Label>
                  <Textarea v-model="form.ru.description" rows="3" placeholder="Описание элемента..." />
                </div>
              </div>
              ...
              <div v-show="activeTab === 'en'" class="space-y-4">
                <div class="space-y-2">
                  ...
                  <Input v-model="form.en.name" placeholder="e.g., Towel" />
                  <p v-if="form.errors['en.name']" class="text-destructive text-xs">{{ form.errors['en.name'] }}</p>
                </div>
                <div class="space-y-2">
                  <Label>Description (English)</Label>
                  <Textarea v-model="form.en.description" rows="3" placeholder="Element description..." />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div class="space-y-2">
                ...
                <Input v-model="form.slug" class="font-mono" placeholder="item-slug" />
        ...
        <div class="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle class="text-base">{{ t('standardItems.icon', 'Ikonka') }}</CardTitle>
            </CardHeader>
            <CardContent>
              <div class="space-y-2">
                <Input v-model="form.icon" class="text-center text-2xl" placeholder="🧺" />
                <p class="text-xs text-muted-foreground">Emoji yoki ikon kiriting</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle class="text-base">{{ t('standardItems.sortOrder', 'Tartib raqami') }}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardContent class="pt-6 space-y-2">
              ...
              <Button type="button" variant="outline" class="w-full" as-child>
                <Link :href="route('admin.standard-items.index')">{{ t('common.cancel', 'Bekor qilish') }}</Link>
              </Button>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      {{ t('standardItems.confirmDelete', 'Bu elementni o\'chirishni xohlaysizmi?') }}
                      <strong class="block mt-2">{{ item.uz?.name }}</strong>
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>{{ t('common.cancel', 'Bekor') }}</AlertDialogCancel>
                    <AlertDialogAction @click="deleteItem" class="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                      {{ t('common.delete', 'O\'chirish') }}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
