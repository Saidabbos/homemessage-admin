...
const submit = () => {
  form.post(route('admin.standard-items.store'));
};
...
<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-bold tracking-tight">{{ t('standardItems.new', 'Yangi standart element') }}</h1>
      <p class="text-muted-foreground">{{ t('standardItems.newSubtitle', 'Standart elementni yaratish') }}</p>
    </div>
            ...
            <CardContent>
              ...
              <div v-show="activeTab === 'uz'" class="space-y-4">
                <div class="space-y-2">
                  ...
                  <Input v-model="form.uz.name" @blur="generateSlug" placeholder="Masalan: Sochiq" />
          ...
          <Card>
            <CardContent class="pt-6 space-y-2">
              ...
              <Button type="button" variant="outline" class="w-full" as-child>
                <Link href="/admin/standard-items">{{ t('common.cancel', 'Bekor qilish') }}</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
