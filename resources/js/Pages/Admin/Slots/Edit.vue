...
const form = useForm({
  start_time: formatTimeForInput(props.slot.start_time),
  end_time: formatTimeForInput(props.slot.end_time),
  sort_order: props.slot.sort_order || 0,
  is_active: props.slot.is_active,
});

const submit = () => {
  form.put(route('admin.slots.update', props.slot.id));
};
</script>
  ...
  <div>
    ...
    <div class="mb-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-[#1f2d3d]">{{ t('slots.edit') }}</h1>
          <p class="text-sm text-[#6c757d] mt-1">{{ formatTimeForInput(slot.start_time) }} - {{ formatTimeForInput(slot.end_time) }}</p>
        </div>
      ...
      <form @submit.prevent="submit" class="p-4">
        ...
        <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
          ...
          <button
            ...
            class="px-4 py-2 bg-[#ffc107] text-[#1f2d3d] text-sm font-medium rounded hover:bg-[#e0a800] transition disabled:opacity-50"
          >
