...
const form = useForm({
  ...
  password: '',
  status: props.dispatcher.status,
});
  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground">{{ dispatcher.name }}</p>
      </div>
      <Button variant="outline" as-child>
        <Link :href="route('admin.dispatchers.index')">{{ t('common.back', 'Orqaga') }}</Link>
      </Button>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent class="space-y-4">
              ...
              <div class="space-y-2">
                <Label>{{ t('dispatchers.password', 'Parol') }}</Label>
                <Input type="password" v-model="form.password" placeholder="••••••••" />
                <p class="text-xs text-muted-foreground">{{ t('dispatchers.passwordHint', 'O\'zgartirmaslik uchun bo\'sh qoldiring') }}</p>
          ...
          <Card>
            <CardContent class="pt-6 space-y-2">
              ...
              <Button type="button" variant="outline" class="w-full" as-child>
                <Link :href="route('admin.dispatchers.index')">{{ t('common.cancel', 'Bekor qilish') }}</Link>
              </Button>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      {{ t('dispatchers.confirmDelete', 'Bu dispetcherni o\'chirishni xohlaysizmi?') }}
                      <strong class="block mt-2">{{ dispatcher.name }}</strong>
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>{{ t('common.cancel', 'Bekor') }}</AlertDialogCancel>
                    <AlertDialogAction @click="deleteDispatcher" class="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                      {{ t('common.delete', 'O\'chirish') }}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
