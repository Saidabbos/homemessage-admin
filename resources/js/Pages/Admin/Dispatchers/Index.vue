  ...
  <div>
    ...
    <div class="mb-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-[#1f2d3d]">{{ t('dispatchers.title') }}</h1>
          <p class="text-sm text-[#6c757d] mt-1">{{ t('dispatchers.subtitle') }}</p>
        </div>
        <nav class="mt-2 sm:mt-0">
          <ol class="flex items-center text-sm">
            <li><Link href="/admin/dashboard" class="text-[#007bff]">{{ t('common.home') }}</Link></li>
            <li class="mx-2 text-[#6c757d]">/</li>
            <li class="text-[#6c757d]">{{ t('dispatchers.title') }}</li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="bg-white rounded shadow-sm">
      ...
      <div class="p-0">
        <!-- Table -->
        <div v-if="dispatchers.data.length > 0" class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-[#f8f9fa] border-b border-gray-200">
              <tr>
                <th class="px-4 py-3 text-left font-semibold text-[#6c757d]">#</th>
                <th class="px-4 py-3 text-left font-semibold text-[#6c757d]">{{ t('dispatchers.name') }}</th>
                <th class="px-4 py-3 text-left font-semibold text-[#6c757d]">{{ t('dispatchers.email') }}</th>
                <th class="px-4 py-3 text-left font-semibold text-[#6c757d]">{{ t('dispatchers.phone') }}</th>
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(dispatcher, index) in dispatchers.data" :key="dispatcher.id" class="hover:bg-[#f8f9fa] transition">
                <td class="px-4 py-3 text-[#6c757d]">{{ index + 1 }}</td>
                <td class="px-4 py-3">
                  <div class="flex items-center">
                    <div class="w-8 h-8 rounded-full bg-[#6f42c1] flex items-center justify-center text-white text-sm font-semibold mr-3">
                      {{ dispatcher.name.charAt(0).toUpperCase() }}
                    </div>
                    <span class="font-medium text-[#1f2d3d]">{{ dispatcher.name }}</span>
                  </div>
                </td>
                <td class="px-4 py-3 text-[#6c757d]">{{ dispatcher.email }}</td>
                <td class="px-4 py-3 text-[#6c757d]">{{ dispatcher.phone || '-' }}</td>
                <td class="px-4 py-3">
                  <span
                    v-if="dispatcher.status"
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center justify-center gap-1">
                    <Link
                      :href="route('admin.dispatchers.show', dispatcher.id)"
                    ...
                    <button
                      @click="deleteDispatcher(dispatcher.id)"
        ...
        <div v-else class="p-12 text-center">
          ...
          <p class="text-[#6c757d] mb-4">{{ t('dispatchers.emptyMessage') }}</p>
          <Link
            href="/admin/dispatchers/create"
          ...
          >
            ...
            {{ t('dispatchers.addFirst') }}
          </Link>
        </div>
      </div>

      <!-- Card Footer with Pagination -->
      <div v-if="dispatchers.data.length > 0" class="px-4 py-3 bg-[#f8f9fa] border-t border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div class="text-sm text-[#6c757d]">
            {{ t('common.total') }}: <strong>{{ dispatchers.total }}</strong> {{ t('common.records') }}
          </div>
